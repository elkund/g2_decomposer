...
for l =1:L
    if transform_s == 1
        ...
        title_str = append(title_str,"\int\rho_"+l+"(s)e^{-(s(q/q_0)^" + result.q_powers(l)/result.t_powers(l) + "(\tau/\tau_0))^" + result.t_powers(l) + "}\mathrm{d}x +");
    end
end
...
% for other cases.
y_labels = dictionary([1 0],{"Diffusive component","Ballistic component"});
...
for q = 1:Q
    ...
    for l=1:L
        plot(result.fit_eval_t,result.f_comp_fits(q,:,l),lstyles{l},'LineWidth',1,'Color',c_map(q,:),'HandleVisibility',lgd_vis,'DisplayName',y_labels{label_keys(l)});
    end
end
